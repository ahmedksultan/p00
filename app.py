...
def edit_name(old_user, new_user):  # function for checking if a new user's username already exists in database
    request.environ.get('HTTP_X_REAL_IP', request.remote_addr)
    ip=request.environ["REMOTE_ADDR"]
    if ip in waitlist:
        waitlist.remove()
    db = sqlite3.connect(DB_FILE)  # open database
    c = db.cursor()
    command = "UPDATE users SET username = \"" + new_user + "\" WHERE username = \"" + old_user + "\""  # check if username is already in database
    c.execute(command)
    return "done"  # if username is not in database


def edit_pwd(old_pwd, new_pwd):  # function for checking if a new user's username already exists in database
    request.environ.get('HTTP_X_REAL_IP', request.remote_addr)
    ip=request.environ["REMOTE_ADDR"]
    if ip in waitlist:
        waitlist.remove()
    db = sqlite3.connect(DB_FILE)  # open database
    c = db.cursor()
    command = "UPDATE users SET password = \"" + new_pwd + "\" WHERE password = \"" + old_pwd + "\""  # check if username is already in database
    c.execute(command)
    return "done"  # if username is not in database
...
def update_check():
    ...
    if(request.form.get('change_user')):
        if (username != None and old_username == "") or (username == "" and old_username != None):
    ...
    if(request.form.get('change_pwd')):
        if (password != None and old_password == "") or (password == "" and old_password != None):
